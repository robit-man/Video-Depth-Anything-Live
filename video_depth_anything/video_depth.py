...
class VideoDepthAnything(nn.Module):
    ...
    def forward(self, x):
        ...
        features = self.pretrained.get_intermediate_layers(
            x.flatten(0, 1), self.intermediate_layer_idx[self.encoder],
    ...
    def infer_video_depth(self, frames, target_fps, input_size=518, device='cuda', fp32=False):
        ...
        # Use frames.shape[0] if available, otherwise fall back to len(frames)
        n_frames = frames.shape[0] if hasattr(frames, 'shape') else len(frames)
        ...
        for frame_id in tqdm(range(0, org_video_len, frame_step)):
            ...
            for i in range(INFER_LEN):
                img = frame_list[frame_id + i].astype(np.float32) / 255.0
                transformed = transform({'image': img})['image']
                # Shape: [C, H, W] then add batch and temporal dims
                cur_list.append(torch.from_numpy(transformed).unsqueeze(0).unsqueeze(0))
